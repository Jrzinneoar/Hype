...
if(method != "info" && method != "delete" && method != "deleteall" && method != "add") {
  ...
  const limite = new Discord.MessageEmbed()
  .setColor('ffffff')
  .setDescription(`Não encontrei canais de levels na minha database !`)
  .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
  .setTimestamp();
...
db.delete(`levelschannel_${message.guild.id}_${temvip[i].ID.split("_")[2]}`)
 ...
 return  message.reply({embeds:[a]})
  ...
  .setColor('ffffff')
  .setDescription(`Não encontrei esse canal de level na minha database !`)
  .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
  .setTimestamp();
...
.setThumbnail(message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
 ...
 message.reply({embeds:[embed]})
  ...
  .setTimestamp();
    ...
    const limite = new Discord.MessageEmbed()
    .setColor('ffffff')
    .setDescription(`A lista de canais de levels está cheia. Use ${prefix}lc delete para deletar.`)
    .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
    .setTimestamp();
  ...
  .setColor('ffffff')
  .setDescription(`Este canal já está na lista como canal de level`)
  .setFooter(`${message.guild.name}`,message.guild.iconURL({ dynamic: true, format: "png", size: 1024 }))
  .setTimestamp();
